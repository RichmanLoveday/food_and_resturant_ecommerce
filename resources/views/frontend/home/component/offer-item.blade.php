                    ...
                    <div class="fp__offer_item_single">
                        ...
                        <div class="text">
                            @if ($dailyOffer->product->offer_price > 0)
                                <span>
                                    {{ discountInPercent($dailyOffer->product->price, $dailyOffer->product->offer_price) }}
                                    % off
                                </span>
                            @endif
                            <a class="title" href="menu_details.html">{!! $dailyOffer->product->name !!}</a>
                            <p>{!! $dailyOffer->product->short_description !!}</p>
                            <ul class="d-flex flex-wrap">
                                <li><a href="javascript:;"
                                        onclick="loadProductModal(this, '{{ $dailyOffer->product->id }}')"><i
                                            class="fas fa-shopping-basket"></i></a></li>
                                <li><a onclick="addToWishList(this, '{{ $dailyOffer->product->id }}')"
